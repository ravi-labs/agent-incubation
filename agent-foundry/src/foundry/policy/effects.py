"""
Financial Services Effect Taxonomy for Agent Foundry.

Effects are the atomic actions an agent can take. Every agent tool call
must declare an effect, which determines the default ALLOW/ASK/DENY
decision and how it is represented in the audit trail.

The taxonomy is organized into six tiers:
...
Each effect maps to a base Tollgate Effect (READ/WRITE/NOTIFY/DELETE)
so it is fully compatible with Tollgate's ControlTower and YAML policy
engine — no changes to Tollgate internals required.
"""
...
class EffectMeta:
    """Metadata for a single financial effect."""
    effect: "FinancialEffect"
    tier: EffectTier
    base_effect: Effect          # Maps to Tollgate's Effect enum
    default_decision: DefaultDecision
    description: str
    requires_human_review: bool  # True = always goes to human review queue
    audit_required: bool = True  # Always true for financial services
...
_EFFECT_META: dict[FinancialEffect, EffectMeta] = {
...
def effects_by_tier(tier: EffectTier) -> list[FinancialEffect]:
    """Return all effects in a given tier."""
    return [e for e, m in _EFFECT_META.items() if m.tier == tier]


def effects_requiring_review() -> list[FinancialEffect]:
    """Return all effects that require human review by default."""
    return [e for e, m in _EFFECT_META.items() if m.requires_human_review]
